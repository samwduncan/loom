...
requires: []
provides:
  - Vite + React 19 + TypeScript project scaffold in src/
...
## Accomplishments
- V2 project bootstrapped in src/ with working Vite dev server, HMR, and backend proxy
...
- `src/vite.config.ts` - Vite config with React, Tailwind v4 plugin, proxy, @ alias
- `src/tsconfig.app.json` - Strict TS with noUncheckedIndexedAccess, @/* paths
- `src/src/main.tsx` - React 19 createRoot entry, imports styles/index.css
- `src/src/App.tsx` - BrowserRouter with / and /dev/tokens routes (named export)
- `src/src/utils/cn.ts` - clsx + tailwind-merge className composition utility
- `src/src/styles/index.css` - Tailwind v4 entry with @theme and @theme inline
- `src/src/styles/base.css` - @font-face declarations, body reset (14px Inter)
- `src/src/styles/tokens.css` - Placeholder dark theme tokens for build viability
- `src/eslint.config.js` - ESLint flat config with Constitution skeleton rules
- `src/public/fonts/*.woff2` - 4 self-hosted font files
