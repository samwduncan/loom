            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm text-foreground-secondary">Initialize Git repository</span>
                </label>
                ...
                <label className="flex items-center gap-3">
                  <input
                    ...
                    className="w-4 h-4 text-primary rounded focus:ring-2 focus:ring-primary"
                  />
                  <span className="text-sm text-foreground-secondary">Store tasks in Git</span>
                </label>
                ...
                <label className="flex items-center gap-3">
                  <input
                    ...
                    className="w-4 h-4 text-primary rounded focus:ring-2 focus:ring-primary"
                  />
                  <span className="text-sm text-foreground-secondary">Add shell aliases (tm, taskmaster)</span>
                </label>
              </div>
                  ...
                  {['claude', 'vscode', 'roo', 'cline', 'windsurf'].map(rule => (
                    <label key={rule} className="flex items-center gap-2">
                      <input
                        ...
                        className="w-4 h-4 text-primary rounded focus:ring-2 focus:ring-primary"
                      />
                      <span className="text-sm text-foreground-secondary capitalize">{rule}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-status-info/10 border border-status-info/20 rounded-lg p-4">
              <div className="flex items-start gap-3">
                <AlertCircle className="w-5 h-5 text-status-info mt-0.5" />
                <div>
                  ...
                  <div className="bg-surface-raised rounded border border-border/10 p-3 mb-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-mono text-muted-foreground">.mcp.json</span>
                      <button
                        onClick={copyMCPConfig}
                        className="flex items-center gap-1 px-2 py-1 text-xs bg-surface-elevated text-muted-foreground rounded hover:bg-muted transition-colors"
                      >
    ...
    "task-master-ai": {
        ...
        "PERPLEXITY_API_KEY": "your_perplexity_key_here"
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <a
                      ...
                      className="text-status-info hover:text-status-info/80 flex items-center gap-1"
                    >
            ...
            <div className="bg-surface-raised rounded-lg p-4">
              ...
              <div className="flex items-center gap-2">
                {setupData.mcpConfigured ? (
                  <>
                    <CheckCircle className="w-4 h-4 text-status-connected" />
                    <span className="text-sm text-status-connected">MCP server is configured</span>
                  </>
                ) : (
                  <>
                    <AlertCircle className="w-4 h-4 text-status-reconnecting" />
                    <span className="text-sm text-status-reconnecting">MCP server not detected (optional)</span>
                  </>
                )}
              </div>
            </div>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div className="text-center">
              <FileText className="w-12 h-12 text-status-connected mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-foreground mb-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={12}
                  className="w-full px-3 py-2 border border-border/10 rounded-md focus:ring-2 focus:ring-primary focus:border-primary bg-surface-raised text-foreground font-mono text-sm"
...
- Phase 2: Enhancements"
              ...
              <div className="bg-status-info/10 border border-status-info/20 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <Sparkles className="w-5 h-5 text-status-info mt-0.5" />
                  <div>
                    <h4 className="font-medium text-foreground mb-1">
                      AI Task Generation
                    </h4>
      ...
      case 4:
        return (
          <div className="space-y-6">
            <div className="text-center">
              <CheckCircle className="w-12 h-12 text-status-connected mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-foreground mb-2">
                Complete Setup
              </h3>
              <p className="text-muted-foreground">
            ...
            <div className="bg-status-connected/10 border border-status-connected/20 rounded-lg p-4">
              <h4 className="font-medium text-status-connected mb-3">
                Setup Summary
              </h4>
              <ul className="space-y-2 text-sm text-status-connected">
  ...
  return (
    <div className="modal-backdrop fixed inset-0 flex items-center justify-center z-[100] md:p-4 bg-black/50">
      <div className={cn(
        ...
        {/* Progress Bar */}
        <div className="px-4 md:px-6 py-4 border-b border-border/10">
          <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          ...
          {error && (
            <div className="mt-4 bg-status-error/10 border border-status-error/30 rounded-lg p-4">
              <div className="flex items-start gap-3">
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-t border-border/10 flex-shrink-0">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-foreground-secondary bg-surface-raised hover:bg-surface-elevated rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            onClick={handleNext}
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-white rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
